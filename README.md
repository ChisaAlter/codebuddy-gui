...
**当前版本：[1.0.1](https://github.com/ChisaAlter/codebuddy-gui/releases/tag/v1.0.1)** · [变更说明](./RELEASE_NOTES.md) · [全部发行版](https://github.com/ChisaAlter/codebuddy-gui/releases)
...
先确保本机可以直接运行 `codebuddy`，且版本 **不低于 GUI 最低支持版本（当前 `2.122.0`）**。设置页的「CodeBuddy CLI 维护」会显示兼容状态；版本过低或缺失时，GUI **拒绝启动项目运行时**，并可一键安装推荐版本。
...
- 前端产物：`out/dist/`
- Windows 安装包：`dist/CodeBuddy-GUI-Setup-<version>.exe`（例如 `CodeBuddy-GUI-Setup-1.0.1.exe`）
...
随后可用 `gh release create v<version> ...` 将上述资产上传到 GitHub Releases（见已发布的 [v1.0.1](https://github.com/ChisaAlter/codebuddy-gui/releases/tag/v1.0.1)）。
