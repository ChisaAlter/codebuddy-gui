...
该命令会运行生产构建，检查 Windows 代码签名，并在 `dist/` 中生成连字符资产名、`latest.yml`、`SHA256SUMS.txt` 和 Release 说明。正式发布默认要求受信任的非自签名证书；可通过 `CODEBUDDY_SIGNER_SUBJECT` 进一步约束发布者名称。没有可用签名证书时，可显式允许未签名安装包：
