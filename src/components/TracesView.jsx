...
export default function TracesView() {
    ...
    React.useEffect(() => {
        fetch(`${API}/api/v1/traces?offset=0&limit=50`, { headers: { 'X-CodeBuddy-Request': '1' } })
            .then(r => r.ok ? r.json() : []).then(setTraces).catch(() => setTraces([]));
    }, []);
    ...
    return (
        <div className="flex flex-col h-full" style={{ background: 'var(--color-bg-primary)' }}>
            ...
            <div className="flex-1 overflow-y-auto p-5">
                {traces.length === 0 ? (
                    <div className="text-center" style={{ color: 'var(--color-text-muted)', marginTop: 60 }}>No traces recorded</div>
                ) : (
                    <div className="rounded-lg overflow-hidden" style={{ border: '1px solid var(--color-border-muted)' }}>
                        <table className="w-full text-sm">
                            <thead><tr>
                                <th className="text-left p-3 text-xs font-medium" style={{ color: 'var(--color-text-muted)', background: 'var(--color-bg-card)' }}>Trace ID</th>
                                <th className="text-left p-3 text-xs font-medium" style={{ color: 'var(--color-text-muted)', background: 'var(--color-bg-card)' }}>Service</th>
                                <th className="text-left p-3 text-xs font-medium" style={{ color: 'var(--color-text-muted)', background: 'var(--color-bg-card)' }}>Duration</th>
                            </tr></thead>
                            <tbody>{traces.map(t => (
