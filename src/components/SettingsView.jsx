            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
                        ...
                        <div className="card p-5">
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                [['dark','Dark'],['light','Light'],['auto','System']].map(([v,l]) => (
                                    <button key={v} onClick={() => setSettings({ theme: v })} className="p-3 rounded-lg text-sm text-center"
                                        style={{ background: settings.theme === v ? 'var(--color-accent-primary-dim)' : 'var(--color-bg-card)', border: `1px solid ${settings.theme === v ? 'var(--color-border-active)' : 'var(--color-border-muted)'}`, color: settings.theme === v ? 'var(--color-accent-brand)' : 'var(--color-text-secondary)' }}>{l}</button>
                                ))}
                            </div>
                        </div>
                        <div className="card p-5">
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                [['zh','中文'],['en','English'],['auto','Auto']].map(([v,l]) => (
                                    <button key={v} onClick={() => setSettings({ language: v })} className="p-3 rounded-lg text-sm text-center"
                                        style={{ background: settings.language === v ? 'var(--color-accent-primary-dim)' : 'var(--color-bg-card)', border: `1px solid ${settings.language === v ? 'var(--color-border-active)' : 'var(--color-border-muted)'}`, color: settings.language === v ? 'var(--color-accent-brand)' : 'var(--color-text-secondary)' }}>{l}</button>
                                ))}
                            </div>
                        </div>
                    </div>
                ...
                {tab === 'model' && (
                    <div className="card p-5">
                        ...
                        <select value={settings.model} onChange={e => setSettings({ model: e.target.value })} className="input-field" style={{ width: 280 }}>
                ...
                {tab === 'permissions' && (
                    <div className="grid grid-cols-2 gap-3">
                        {[
                            ...
                            <button key={p.id} onClick={() => setSettings({ permissionMode: p.id })} className="p-4 rounded-xl text-left"
                                style={{ background: settings.permissionMode === p.id ? 'var(--color-accent-primary-dim)' : 'var(--color-bg-card)', border: `1px solid ${settings.permissionMode === p.id ? 'var(--color-border-active)' : 'var(--color-border-muted)'}` }}>
                ...
                {tab === 'server' && (
                    <div className="card p-5">
                        <h3 className="text-sm font-medium mb-3" style={{ color: 'var(--color-text-primary)' }}>CodeBuddy HTTP Server</h3>
